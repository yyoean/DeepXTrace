...
class TestDiagnose(unittest.TestCase):
    ...
    def test_diagnose_row(self):
        ...
        self.assertEqual(res, {'abnormal_cols': [], 'abnormal_rows': [3], 'abnormal_points': []})

    def test_diagnose_col(self):
        res = ds.Diagnose.diagnose_matrix(self.abnormal_col)
        self.assertEqual(res, {'abnormal_cols': [2, 4], 'abnormal_rows': [], 'abnormal_points': []})

    def test_diagnose_mixed(self):
        res = ds.Diagnose.diagnose_matrix(self.abnormal_mixed, thres_col=1.0, thres_row=1.0)
        self.assertEqual(res, {'abnormal_cols': [2, 4], 'abnormal_rows': [6], 'abnormal_points': []})

    def test_diagnose_point(self):
        res = ds.Diagnose.diagnose_matrix(self.abnormal_point)
        self.assertEqual(res, {'abnormal_cols': [], 'abnormal_rows': [], 'abnormal_points': [(3, 4, 125, 7.279344854723584)]})
